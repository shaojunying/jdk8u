...
GenCollectedHeap::GenCollectedHeap(GenCollectorPolicy *policy) : SharedHeap(policy),
                                                                 _gen_policy(policy),
                                 ...
                                 ReservedSpace *heap_rs)
{
  ...
  assert(total_reserved % alignment == 0,
         err_msg("Gen size; total_reserved=" SIZE_FORMAT ", alignment=" SIZE_FORMAT, total_reserved, alignment));
...
{
  // UseConcMarkSweepGC: true, 使用CMS垃圾收集器
  //
  // 如果是javalangsystemgc并且ExplicitGCInvokesConcurrent为 true 则返回 true
...
bool GenCollectedHeap::is_in(const void *p) const
{
  ...
  guarantee(VerifyBeforeGC ||
                ...
                VerifyAfterGC ||
                VMError::fatal_error_in_progress(),
            "too expensive");
